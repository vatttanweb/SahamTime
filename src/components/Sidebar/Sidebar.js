        ...
        {sideBarMenu.map((itm, ind) => (
          <li className="d-flex pr-4 mt-3">
            <Link className={s["sideBar-link"]} to={itm.route}>
              ...
              <div className="d-flex justify-content-center align-items-center mx-1">{itm.label}</div>
            </Link>
          </li>
        ))}
...
let sideBarMenu = [
  ...
  },
  {
    label: "جدول ها",
    route: "/login",
    icon: <TablesIcon className={s.menuIcon} />,
  },
  {
    label: "اعلانات",
    route: "/login",
    icon: <NotificationsIcon className={s.menuIcon} />,
  },
  {
    label: "کامپوننت",
    route: "/login",
    icon: <ComponentsIcon className={s.menuIcon} />,
  },
  // {
  //   label: "خانه",
  //   route: "/login",
